...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['slots', 'conversation', 'sessions'], (scope: ClientContext) => {
    ...
    // Optional workspaces service: present in every composed web client;
    // read through ctx.get so an absent service degrades to the cwd path.
    const workspaces = ctx.get('workspaces')

    /** The session's owning workspace path, resolved at call time from the workspaces registry (the host gate compares this canon directly). */
    const workspacePathOf = (sessionId: SessionId | undefined): string | undefined =>
      sessionId === undefined ? undefined
        : workspaces?.list.getSnapshot().items.find(item => item.sessionIds.includes(sessionId))?.path
    ...
    const injected = (): GitGraphInjected => {
      /** Resolve the workspace root for one git call: the owning workspace's registered path first (the branch views must reflect THIS workspace's repository even when the session cwd is a subdirectory or briefly stale), the session cwd as the fallback. */
      const pathOf = (sessionId: SessionId | undefined): { ok: true; path: string } | { ok: false; error: GitError } => {
        const path = workspacePathOf(sessionId) ?? cwdOf(sessionId)
        if (path === undefined || path === '') return { ok: false, error: NO_WORKSPACE }
        return { ok: true, path }
      }
      return {
