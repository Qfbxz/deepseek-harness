...
export const StatsLine = memo(function StatsLine({ useSession, useProjection, t }: StatsLineProps) {
  ...
  // Two pipe-separated rows (figma stats strip): step-derived activity on the
  // first, billing on the second; a group with no data drops out whole and an
  // empty row does not render. Rows wrap within the message column instead of
  // eliding, so every figure stays visible.
  const activity: string[] = []
  const billing: string[] = []
  if (stats.steps > 0) {
    activity.push(t('stats.counts', { turns: stats.turns, steps: stats.steps }))
    const durations: string[] = []
    if (stats.llmMs > 0) durations.push(t('stats.llm', { duration: formatDuration(stats.llmMs) }))
    if (stats.toolMs > 0) durations.push(t('stats.toolCall', { duration: formatDuration(stats.toolMs) }))
    if (durations.length > 0) activity.push(durations.join(' · '))
    const speeds: string[] = []
    ...
    if (speeds.length > 0) activity.push(speeds.join(' · '))
  }
  ...
  if (usage !== undefined
    ...
    if (cacheHit !== null) billing.push(t('stats.cacheHit', { percent: cacheHit }))
    billing.push(t('stats.tokens', {
      input: formatTokens(billedInputTokens(usage)),
      output: formatTokens(usage.outputTokens),
    }))
  }
  const rows = [activity, billing].filter(groups => groups.length > 0)
