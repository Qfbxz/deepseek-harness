...
function deriveBashDescription(command) {
	for (const rawLine of command.split("\n")) {
		...
		return stripped.length > DERIVED_DESCRIPTION_MAX_CHARS ? stripped.slice(0, DERIVED_DESCRIPTION_MAX_CHARS - 1) + "…" : stripped;
	}
...
function presentBashCall(args) {
	...
	return {
		card: "terminal",
		title: args.command,
		description: args.description?.trim() ? args.description : deriveBashDescription(args.command),
