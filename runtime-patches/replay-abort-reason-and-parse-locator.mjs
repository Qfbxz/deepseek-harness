...
if (src.includes('renderAbortReason(request.signal')) { console.log('[skip] already patched:', target); process.exit(0) }
...
if (!src.includes('function renderAbortReason')) {
	let injectAt = src.indexOf('}', msgIdx) + 1
	src = src.slice(0, injectAt) + '\n' + fnAbort + '\n' + fnLocator + src.slice(injectAt)
}

// 3) 改写两处 abort 构造（pre-abort 与 onAbort）
// 2026-08-20: 全局编译副本的对象字面量是多行布局, 用 message: 子串锚同时覆盖
// 多行与单行两种形态; 改写后校验命中数, 防再次静默半途。
const preOld = 'message: String(request.signal.reason)'
const preNew = 'message: `aborted: ${renderAbortReason(request.signal.reason)}`'
const onOld = 'message: String(request.signal?.reason)'
