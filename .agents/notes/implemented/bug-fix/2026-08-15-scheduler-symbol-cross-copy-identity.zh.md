...
- **去重物理副本**（workspace 别名、仅宿主解析）。属打包策略变更，自有失败模式；身份级注册不动依赖布局即修复跨副本契约。
- **服务对象上的字符串键。** 字符串键与公开方法名冲突并失去符号的不可枚举与非字符串安全性；注册表符号两者兼得且仍与副本无关。

## 影响

任何版本、任何副本，只要采用同一注册方式即可互操作，与 pnpm 物化多少副本、某组合解析到哪个副本无关。注册表键 `@deepseek-ai/dsh-tools.scheduler` 自此成为已发布契约：日后重命名会破坏混合副本间的跨版本互操作，故其被包文档化并保持稳定。
