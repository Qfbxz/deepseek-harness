...
在 `'code'` 和 `'both'` 下，注册表拥有 `run_code` 作为保留的呈现传输通道，参数为 `{ code: string; description?: string }`——`code` 必填，`description` 可选（缺失或为空白时从程序首个有效行派生卡片标题，见 [会话摩擦修复笔记](../../bug-fix/2026-08-16-session-friction-fixes.md)；沿用 bash 先例的标注是兜底而非门槛）。它由一个正常的 `ToolDefinition` 表示以供分发，但位于可过滤的能力层之外，因此限制规则不会意外移除 Code Mode 的唯一入口。调用遍历完整的工具流水线——`tools/pre-execute` → 单调性守卫 → `tools/execute` 包裹分发 → `tools/post-execute` → 由定义拥有的可选 `finalizeContent` → 不可变的 `tools/result` 通知——与原生调用完全一致；权限插件可以在程序运行前检查程序文本，最终结果观察者看到的是规范化的外层结果。其 `execute(args, exec)`：
...
传输自身的 `description` 与两种 flavor 的 SDK 说明开头同时点名 `code` 和 `description`。把该调用描述成「传入一个程序」的散文会让第二个参数只能从参数 schema 中发现；自 2026-08-16 起 schema 将其标记为可选但建议提供，`presentCall` 会派生标题，只发出 `{code}` 的模型仍能完成运行，不再因 `INVALID_ARGS` 连同已写好的程序一起丢失。
